...
export function normalizePaths(data){
 if(data.error){const code=String(data.error.code);throw new TransitError(code,['-99','-98','3','4','5'].includes(code)?'이 구간의 대중교통 경로를 찾지 못했어요. 가까운 지점은 보행 경로를 확인해 주세요.':'교통 정보 제공처에서 조회를 완료하지 못했어요.',422);}
...
export async function handleTransit(request,env={},fetcher=fetch){
 ...
 try{
  ...
  if(url.pathname==='/api/transit/routes'){
   ...
   const data=await call('searchPubTransPathT',{SX:body.from[0],SY:body.from[1],EX:body.to[0],EY:body.to[1],SearchType:0,SearchPathType:{all:0,subway:1,bus:2}[body.mode]});return respond({routes:normalizePaths(data),source:'ODsay',checkedAt:new Date().toISOString()});
  }
  if(url.pathname==='/api/transit/geometry'){
   ...
   const data=await call('loadLane',{mapObject:'0:0@'+body.mapObj});if(data.error)throw new TransitError('GEOMETRY','노선 선형을 불러오지 못했어요. 구간 안내는 계속 볼 수 있어요.');
