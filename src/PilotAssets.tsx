...
export default function PilotAssets({initial='station',onBack,onLegacy,onPlace}:{initial?:PilotAssetId;onBack:()=>void;onLegacy:()=>void;onPlace:(asset:PilotAssetId)=>void}){
 ...
 return <div className={'city-app asset-workspace'+(panelOpen?'':' panel-collapsed')}>
  <header className="dj-header"><button className="dj-brand" onClick={onBack}><img src="/icon-192.png" alt=""/><strong>갈수있슈<small>나에게 맞는 대전 여행길</small></strong></button><div className="dj-location"><i/>대전광역시 <span>3D 모형</span></div><button className="dj-profile" onClick={onBack}>← 지도로 돌아가기</button></header>
